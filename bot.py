...
class KeyTypeSelect(discord.ui.Select):
    def __init__(self):
        options = [
            ...
            discord.SelectOption(label="Month Key", description="30 days access - $20", emoji="📅", value="monthly"),
            discord.SelectOption(label="Lifetime Key", description="365 days access - $50", emoji="⭐", value="lifetime")
        ]
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        selected_name = key_names[self.values[0]]
        selected_price = price_map[self.values[0]]

        embed = discord.Embed(
            title="💳 Select Payment Method",
            description=f"You selected: **{selected_name}** (${selected_price})\n\nChoose your cryptocurrency:",
            color=0x5a3e99
        )
        await interaction.response.edit_message(embed=embed, view=self.view.get_crypto_view())
...
class CryptoSelect(discord.ui.Select):
    def __init__(self, key_type: str):
        ...
        options = [
            ...
            discord.SelectOption(label="LTC", description=f"Pay ${price} with Litecoin", emoji="⚪", value="LTC"),
            discord.SelectOption(label="ETH", description=f"Pay ${price} with Ethereum", emoji="🔵", value="ETH")
        ]
...
class AutobuyView(discord.ui.View):
    ...
    def get_crypto_view(self):
        ...
        async def back_callback(button_interaction):
            ...
            await button_interaction.response.edit_message(embed=embed, view=original_view)
        ...
        try:
            ...
            try:
                ...
                if r.status_code not in (200, 201):
                    await interaction.edit_original_response(content=f"❌ Failed to create invoice (HTTP {r.status_code}).", view=None)
                    return
                inv = r.json()
            except Exception as e:
                await interaction.edit_original_response(content=f"❌ Error creating invoice: {e}", view=None)
                return

            url = inv.get("invoice_url") or inv.get("pay_url")
            if not url:
                await interaction.edit_original_response(content="❌ Invoice created but no URL returned.", view=None)
                return
