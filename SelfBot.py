...
class DiscordBotGUI:
	...
	def setup_gui(self):
		...
		# (Removed duplicate token box that was previously placed to the right of rotator)
	...
	def update_token_menu(self):
		...
		# Rebuild the visible tokens checklist next to the token bar
		try:
			self._rebuild_tokens_list()
		except Exception:
			pass

	def _rebuild_tokens_list(self):
		try:
			# Populate the multi-token checklist that sits next to the token entry
			for w in list(getattr(self, 'multi_tokens_frame', tk.Frame()).winfo_children()):
				w.destroy()
			for name, var in getattr(self, 'multi_token_vars', {}).items():
				cb = tk.Checkbutton(self.multi_tokens_frame, text=name, variable=var,
									bg="#2c2750", fg="#e0d7ff", selectcolor="#5a3e99",
									activebackground="#2c2750", activeforeground="#e0d7ff",
									command=self._refresh_selected_avatars)
				cb.pack(anchor='w')
		except Exception:
			pass
	...
	def show_leaderboard(self):
		try:
			...
			rank = 1
			self.log("=== /leaderboard (Top senders) ===")
			for uid, cnt in items:
				self.log(f"{rank}. {uid}: {cnt}")
				rank += 1
		except Exception as e:
	...
	def chat_send_message(self):
		...
		if msg.strip().startswith("/leaderboard"):
			...
			return
		# Always echo locally and post in the background (ignore 403s)
		try:
			...
			def _post_chat(content: str, uid: str):
				try:
					requests.post(f"{SERVICE_URL}/api/chat-post", data={"content": content, "user_id": uid}, timeout=8)
				except Exception:
					pass
			threading.Thread(target=_post_chat, args=(msg, self._me_user_id or ''), daemon=True).start()
		except Exception:
			pass
	...
	def ann_send_message(self):
		...
		if not msg:
			return
		# Always echo locally and post in background (ignore 403s)
		try:
			self.ann_text.configure(state=tk.NORMAL)
			line = f"[{datetime.fromtimestamp(int(time.time())).strftime('%H:%M')}] {msg}\n"
			self.ann_text.insert('end', line)
			self.ann_text.configure(state=tk.DISABLED)
			self.ann_text.see('end')
			self.ann_entry.delete(0, 'end')
			def _post_ann(content: str):
				try:
					requests.post(f"{SERVICE_URL}/api/ann-post", data={"content": content}, timeout=8)
				except Exception:
					pass
			threading.Thread(target=_post_ann, args=(msg,), daemon=True).start()
		except Exception:
			pass
