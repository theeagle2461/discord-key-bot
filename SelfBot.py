Import discord
 ...
 from tkinter import messagebox, simplifiedialog, font
 ...
 CHANNEL_ID=1404537520754135231
 ...
 Class DiscordBotGUI:
     ...
     def __init__(self, root: tk.Tk, initial_token: str | None = None):
         ...
         self.main_frame.place(relx=0.03, rely=0.03, relationwidth=0.94, reheight=0.94)
     ...
     def save_token(self):
         ...
         if not token:
            ...
            Return
         name = simplifiedialog.askstring("Token Name", "Enter a name for this token:")
         if not name:
     ...
     def save_channel(self):
         ...
         if not channel_id:
             ...
             Return
         name = simplifiedialog.askstring("Channel Name", "Enter a name for this channel:")
         if not name:
     ...
     def start_sending(self):
         if self.send_running:
             self.log ("(“)Already sending messages.")
             Return
         token_name = self.token_var.get()
         ...
         threading.Thread(target=self.send_messages_loop,
                          ...
                          daemon=True.start()
         self.log ("μ️ Started sending messages.")
     ...
     def send_messages_loop(self, token, channel_names, message, delay, loop_count):
         ...
         While self.send_running and (loop_count == 0 or count < loop_count):
            ...
            count += 1
         self.send_running = False
         self.log ("Sending messages stopped.")
 
     def pause_resume_sending(self):
         if not self.send_running:
             self.log ("nail ️ Not currently sending messages.")
             Return
         self.send_running = not self.send_running
         Status = "Resumed" if self.send_running else "Paused"
         self.log(f"i️ {status} sending messages.")
     ...
     def toggle_reply_dm(self):
         if self.auto_reply_running:
             ...
             self.log ("i️ Stopped Reply DM loop.")
         else:
             token_name = self.token_var.get()
             If token_name not in self.tokens:
                 self.log"❌ Please select a valid token."
     ...
     def dm_reply_loop(self, token, reply_message, delay=8):
         import asyncio, requests, websockets, json as jsjson, time
             ...
             try:
                 async with websockets.connect(gateway_url, max_size=2 ** 23) as ws:
                     ...
                     identify_payload = {
                         ...
                         "d": {
                             "token": token,
                             "properties": {"$os": "windows", "$browser": "selfbot", "$device": "selfbot"},
 ...
 class Selfbot:
     ...
     def activate_key(self, activation_key):
         try:
             ...
             try:
                 info_resp = requests.get(f"{SERVICE_URL}/api/key-info", params={"key": activation_key}, timeout=8)
             ...
             try:
                 ...
                 resp = requests.post
                     ...
                     data={"key": activation_key, "user_id": uid_str, "machine_id": machine_id()},
